...
export default function PricesPage() {
  ...
  const handleDeleteProduct = (iIdx) => {
    const newData = [...data];
    newData.splice(iIdx, 1);
    saveData(newData);
  };

  const handleDeleteStore = (iIdx, sIdx) => {
    const newData = [...data];
    newData[iIdx].stores.splice(sIdx, 1);
    if (newData[iIdx].stores.length === 0) {
      newData.splice(iIdx, 1);
    }
    saveData(newData);
  };
  ...
  const openEditStoreModal = (iIdx, sIdx) => {
    const store = data[iIdx].stores[sIdx];
  ...
  const handleSave = () => {
    ...
    if (modalMode === 'new_product') {
      let foundExisting = newData.find(i => i.name.toLowerCase() === newName.trim().toLowerCase());
      if (foundExisting) {
      ...
      const item = newData[editingItem.iIdx];
      ...
      if (item.name === newName.trim()) {
        ...
        // Name changed -> move store to new product group
        item.stores.splice(editingItem.sIdx, 1);
        if (item.stores.length === 0) {
          newData.splice(editingItem.iIdx, 1);
        }
    ...
    <div className="tasks-page">
      ...
      <div className="tasks-body" style={{ padding: '0 16px', gap: '12px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div key={item.originalIndex} style={{ 
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                <button 
                  onClick={() => openAddStoreModal(item.originalIndex)}
                ...
                <button 
                  onClick={() => handleDeleteProduct(item.originalIndex)}
              ...
              {item.stores.map((store, sIdx) => (
                <div key={sIdx} style={{ 
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '4px' }}>
                    <button 
                      onClick={() => openEditStoreModal(item.originalIndex, sIdx)}
                    ...
                    <button 
                      onClick={() => handleDeleteStore(item.originalIndex, sIdx)}
        ...
        {filteredItems.length === 0 && (
          <div style={{ textAlign: 'center', color: 'var(--text-muted)', marginTop: '20px' }}>Ничего не найдено</div>
        )}
      </div>
